...
// -----------------------------------------------------------------------------
// FileSystemHelper - Pure static, stateless utility class for ShaderBundle
// -----------------------------------------------------------------------------
...
public:
    ...
    // Directory scanning - Returns only subdirectories (not files)
    ...
    // Existence checks - Wrappers for std::filesystem operations
    static bool FileExists(const std::filesystem::path& path);
    static bool DirectoryExists(const std::filesystem::path& path);

    // Path composition - Combines and normalizes paths
    static std::filesystem::path CombinePath(
        const std::filesystem::path& base,
        const std::string&           relative);

    // ByteBuffer file I/O - Binary read/write with FileIOException on failure
