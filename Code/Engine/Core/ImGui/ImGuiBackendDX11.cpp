...
ImGuiBackendDX11::ImGuiBackendDX11(IRenderer* renderer)
    : m_device(nullptr)
    , m_context(nullptr)
    , m_swapChain(nullptr)
    , m_mainRenderTargetView(nullptr)
{
    ...
    if (renderer)
    {
        m_device = renderer->GetD3D11Device();
        m_context = renderer->GetD3D11DeviceContext();
        m_swapChain = renderer->GetD3D11SwapChain();
    }

    // 不持有设备的所有权，只保存指针
    // 设备生命周期由外部管理（通常是Renderer）
}
