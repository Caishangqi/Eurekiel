...
SoundPlaybackID SoundResource::Play(AudioSystem& audioSystem, bool isLooped, float volume, float balance, float speed, bool isPaused) const
{
    UNUSED(audioSystem); UNUSED(isLooped); UNUSED(volume); UNUSED(balance); UNUSED(speed); UNUSED(isPaused);
    return MISSING_SOUND_ID;
}

SoundPlaybackID SoundResource::PlayAt(AudioSystem& audioSystem, const Vec3& position, bool isLooped, float volume, float balance, float speed, bool isPaused) const
{
    UNUSED(audioSystem); UNUSED(position); UNUSED(isLooped); UNUSED(volume); UNUSED(balance); UNUSED(speed); UNUSED(isPaused);
    return MISSING_SOUND_ID;
}
...
void SoundResource::Unload() {}
bool SoundResource::Reload(const std::vector<uint8_t>& data, const SoundConfig& config) { UNUSED(data); UNUSED(config); return false; }
void SoundResource::CacheRawData() const {}
