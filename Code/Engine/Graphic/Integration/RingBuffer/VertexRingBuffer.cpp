...
namespace enigma::graphic
{
    ...
    VertexAppendResult VertexRingBuffer::Append(const void* vertices, size_t vertexCount, size_t stride)
    {
        ...
        {
            ERROR_AND_DIE("VertexRingBuffer: frame partition overflow");
        }
    ...
    //-------------------------------------------------------------------------------------------
    void VertexRingBuffer::ResetForFrame()
    {
        ...
        if (IsPreAcquireFramePhase(phase))
        {
            ERROR_RECOVERABLE(Stringf(
                "VertexRingBuffer::ResetForFrame called before frame slot acquisition (phase=%s, name=%s)",
