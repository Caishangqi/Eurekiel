...
private:
    ...
    // 设备和队列
    ID3D12Device*           m_device;               // DX12设备
    ID3D12CommandQueue*     m_graphicsQueue;       // 图形命令队列
    ID3D12CommandQueue*     m_computeQueue;        // 计算命令队列
    ID3D12CommandQueue*     m_copyQueue;           // 复制命令队列
    
    // 同步对象
    ID3D12Fence*            m_fence;               // 围栏对象
...
public:
     ...
     * @param graphicsQueue 图形命令队列
     * @param computeQueue 计算命令队列 (可为nullptr)
     * @param copyQueue 复制命令队列 (可为nullptr)
    ...
    bool Initialize(ID3D12Device* device,
                   ...
                   uint32_t graphicsCount = DEFAULT_GRAPHICS_LIST_COUNT,
     ...
     */
    uint64_t ExecuteCommandLists(ID3D12GraphicsCommandList** commandLists, uint32_t count);
