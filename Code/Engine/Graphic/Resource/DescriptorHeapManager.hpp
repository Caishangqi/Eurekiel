...
public:
     ...
     */
    enum class HeapType {
        CBV_SRV_UAV,    // 常量缓冲区、着色器资源、无序访问视图堆
        Sampler         // 采样器堆
    };
...
private:
    ...
    // 核心描述符堆
    std::unique_ptr<DescriptorHeap> m_cbvSrvUavHeap;    // CBV/SRV/UAV堆
    ...
    // 默认配置
    static constexpr uint32_t DEFAULT_CBV_SRV_UAV_CAPACITY = 1000000;  // 默认100万个CBV/SRV/UAV
...
public:
    ...
    /**
     * @brief 检查是否还有足够空间
     * @param cbvSrvUavCount 需要的CBV/SRV/UAV数量
     * @param samplerCount 需要的Sampler数量
     * @return 有足够空间返回true
     */
    bool HasEnoughSpace(uint32_t cbvSrvUavCount, uint32_t samplerCount = 0) const;
