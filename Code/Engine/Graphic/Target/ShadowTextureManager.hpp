 ...
 * - DX12: ShadowTextureManager（D12Texture + Bindless集成）
 *
 * 职责边界:
 * - ShadowTextureManager: 管理ShadowTex纹理（只读）
...
namespace enigma::graphic
{
    ...
    public:
        ...
        /**
         * @brief 构造ShadowTextureManager并创建ShadowTex纹理
        ...
        ~ShadowTextureManager() = default;

        // 禁用拷贝和移动（Manager类通常作为单例使用）
        ShadowTextureManager(const ShadowTextureManager&)            = delete;
        ShadowTextureManager& operator=(const ShadowTextureManager&) = delete;
        ShadowTextureManager(ShadowTextureManager&&)                 = delete;
        ShadowTextureManager& operator=(ShadowTextureManager&&)      = delete;
