...
namespace enigma::graphic
{
     ...
     * **示例**：
     * ```cpp
     * auto path1 = ShaderPath::FromAbsolutePath("/shaders/gbuffers_terrain.hlsl");
    ...
    public:
        /**
         * @brief 从绝对路径字符串创建 ShaderPath
         * @param absolutePath 必须以 `/` 开头的绝对路径
         * @return 标准化后的 ShaderPath
         ...
         * - 自动标准化（处理 `.` 和 `..`）
         */
        static ShaderPath FromAbsolutePath(const std::string& absolutePath);
         ...
         * - `/` → std::nullopt
         */
        std::optional<ShaderPath> Parent() const;
         ...
         * - 结果: `/shaders/lib/common.hlsl`
         */
        ShaderPath Resolve(const std::string& relativePath) const;
        ...
        // 标准比较运算符
        // ========================================================================
        bool operator==(const ShaderPath& other) const { return m_path == other.m_path; }
        bool operator!=(const ShaderPath& other) const { return m_path != other.m_path; }

        // ========================================================================
        // 调试支持
        // ========================================================================
        std::string ToString() const { return "ShaderPath {" + m_path + "}"; }
    ...
    private:
         ...
         */
        explicit ShaderPath(const std::string& normalizedPath);
...
// ========================================================================
// ShaderPath 哈希支持（用于 unordered_map/set）
// ========================================================================

namespace std
{
    /**
     * @brief std::hash 特化，使 ShaderPath 可以用于 unordered_map/set
     *
     * 教学要点:
     * - std::hash 特化应该与类定义在同一个头文件中
     * - 使用路径字符串的哈希值作为 ShaderPath 的哈希值
     * - 这样 unordered_map<ShaderPath, T> 才能正常工作
     */
    template <>
    struct hash<enigma::graphic::ShaderPath>
    {
        std::size_t operator()(const enigma::graphic::ShaderPath& path) const noexcept
        {
...
{
    // 向后兼容别名 - 保持旧代码可以正常工作
    using AbsolutePackPath = ShaderPath;
}
